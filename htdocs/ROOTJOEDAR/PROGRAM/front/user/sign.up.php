...
function userAdd($email,$pass,$password){
	// 校验邮箱是否存在
	$hasEmail = mysql_fetch_array(mysql_query("SELECT * FROM JOEDAR_USERLIST WHERE email = '$email'"));
	// 如果邮箱存在
	if($hasEmail){
	...
	// 插入数据库
	$sql = "INSERT INTO JOEDAR_USERLIST (id,email,pass,password) VALUES (NULL,'$email','$pass','$password')";

	if(mysql_query($sql)){
		$info = mysql_fetch_array(mysql_query("SELECT * FROM JOEDAR_USERLIST WHERE email = '$email'"));
		if($info){
			$data = array(
				'id' => (int)$info['id']
				,'email' => $info['email']
