...
function signinCheck($email,$username,$password){
	...
	//----------------------
	if($email){
		$email_check_query = mysql_query("SELECT * FROM JOEDAR_USERLIST WHERE email='$email' limit 1");
		$email_result = mysql_fetch_array($email_check_query);
		if($email_result){
			$password_check_query = mysql_query("SELECT * FROM JOEDAR_USERLIST WHERE password='$password' limit 1");
	...
	//----------------------
	if($username){
		$username_check_query = mysql_query("SELECT * FROM JOEDAR_USERLIST WHERE username='$username' limit 1");
